...
/*   Updated: 2025/04/22 12:51:35 by roberto          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::partChannel(std::string channelName, int fd)
{
	...
	if (getChannel(channelName)->getMembers().empty())
	{
		...
		std::cout << "\033[31mClient \033[0m" << getClient(fd)->getSocket() << "\033[31m, you are not in the channel \033[0m" << channelName << std::endl;
