...
/*   Updated: 2025/04/21 17:11:03 by roberto          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::partChannel(std::string channelName, int fd)
{
	...
	std::string partMsg = getClient(fd)->getPrefix() + " JOIN :" + channelName + "\r\n";


	send(getClient(fd)->getSocket(), partMsg.c_str(), joinMsg.length(), 0);
}
