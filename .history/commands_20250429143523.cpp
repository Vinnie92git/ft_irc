...
/*   Updated: 2025/04/29 14:35:23 by roberto          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::topicChannel(std::string channelName, std::string topic, int fd)
{
	...
	*/
	...
	// si el usuario no está en el canal significa que no puede ver el topic
	if (!getChannel(channelName))
	{
		std::cout << "\033[31mClient \033[0m" << getClient(fd)->getSocket() << "\033[31m, you are not in \033[0m" << channelName << std::endl;
		return;
	...
	std::cout << "entrasndo, channelname:" << channelName << " topic:" << topic << std::endl;
