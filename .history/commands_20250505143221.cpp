...
/*   Updated: 2025/05/05 14:32:21 by roberto          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::kickUserFromChannel(std::string channelName, std::string user, std::string reason, int fd)
{
	...
	std::string nicknameBuscado = "Vini 42"; // reemplaza con el nickname que buscas
	int socketEncontrado = -1;

	for (Client& client : connectedClients) {
	  if (client.getNickname() == nicknameBuscado) {
		socketEncontrado = client.getSocket();
		break;
	  }
	}

	if (socketEncontrado != -1) {
	  // haz algo con el socket
	} else {
	  std::cout << "No se encontró el socket para el nickname " << nicknameBuscado << std::endl;
	}
